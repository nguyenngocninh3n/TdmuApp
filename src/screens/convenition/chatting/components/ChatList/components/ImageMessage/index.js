...
const ImageMessage = React.memo(
  ({ _id, senderID, message, owner, aka, time, messageTime, avatar, onLongPress, detail }) => {
        ...
        {!owner && <ChatAvatar avatar={API.getFileUrl(avatar)} size={30} />}
          ...
          >
            {message.split(',').map((item, index) => (
              ...
              >
                <Image source={imageList.at(index)} width={'100%'} height={'100%'} style={{borderWidth:1, borderColor:'#ddd'}} resizeMode='cover' />
              </TouchableOpacity>
            ))}
          </View>
        </View>
